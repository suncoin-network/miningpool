...
# Current Author: 	Slunce
#
# Program:
#   Install yiimp on Ubuntu 16.04 running Nginx, MariaDB, and php7.x
#
# 1. startup docker (check if you need any addiontal port)
# docker run -i -p 3306:3306 -p 587:587 -p 11211:11211 -p 80:80 -p 4233:4233 -t ubuntu:latest /bin/bash
# 
# 2. update & install git
# apt-get update && apt-get install -y sudo nano git
# 
# 3. cd ~ 
#
# 3. git clone https://github.com/suncoin-network/pool.git
# 
# 3. change <ip> to your public IP in the script and run:
# sudo sed -i s/PLACEHOLDERYOURIP/<ip>/g  ~/pool/install.sh
# 
# 4. run the install script (check preconfigured)
...
output "Or stop here and modify the file first!"
output ""

TIME="America/New_York"
server_name="localhost"
sub_domain="n"
EMAIL="asodicnw@woeicnew.com"
BTC="N"
admin_panel="/site/adminRights"
Public="PLACEHOLDERYOURIP,172.17.0.1,0.0.0.0,127.0.0.1,172.17.0.4"
install_fail2ban="n"
UFW="n"
ssl_install="n"
...
echo "URL AdminPanel: "$admin_panel
echo "Public IPs: "$Public

output ""
read -e -p "press anykey to proceed installation " TIME
...
#    read -e -p "Enter your Public IP for admin access (http://www.whatsmyip.org/) + ,172.17.0.1,0.0.0.0,127.0.0.1,172.17.0.4 : " Public
...
output "Your mysql information is saved in ~/.my.cnf. this installer did not directly install anything required to build coins."
...
output ""
output "run bash main.sh bash loop2.sh bash block.sh"
output ""
output "debug log vua cat /var/log/debug.log"
